...
export const signAccessToken = (payload: { sub: string; role: string }) => {
  const options: SignOptions = { expiresIn: ACCESS_EXPIRES ? parseInt(ACCESS_EXPIRES) : undefined };
  return jwt.sign(payload, ACCESS_SECRET, options);
};

export const signRefreshToken = (payload: { sub: string; role: string }) => {
  const options: SignOptions = { expiresIn: REFRESH_EXPIRES ? parseInt(REFRESH_EXPIRES) : undefined };
